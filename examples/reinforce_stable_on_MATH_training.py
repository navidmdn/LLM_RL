...
from envs.MATH import MATHEnv, FinalAnswerRewardModel
...
def run(sys_prompt_path: str, model_name_or_path: str, cache_dir: str = None,
        n_few_shot_examples = 8, qlora = False, local_test: bool = False,
        buffer_cache_path: str = None, reward_model_sys_prompt_path: str = None,
        num_iterations: int = 10, num_collection_episodes: int = 1, max_trajectory_steps: int = 15,
        num_update_steps: int = 10, load_buffer_from_cache_at_start: bool = False,
        cache_buffer: bool = False, gradient_accumulation_steps: int = 16,
        save_path: str = "model.pt", evaluation_at_start: bool = False, evaluation_iterations: int = 10,
        n_save_iterations: int = None, n_eval_steps: int = 20, use_lora: bool = False,
        kl_coeff: float = 0.25, buffer_size: int = 10000):
    ...
    env = MATHEnv(reward_model=reward_model, split='train', n_few_shot_examples=n_few_shot_examples, cache_dir=cache_dir)
    evaluation_env = MATHEnv(reward_model=reward_model, split='test', n_few_shot_examples=n_few_shot_examples,
                             cache_dir=cache_dir)
